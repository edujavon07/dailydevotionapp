...
APP_VERSION = "v3.5 (Ultimate Anti-Crash)"
...
def generate_voicebox_audio(text, server_url, voice_preset, selected_engine, output_path, status_callback=None, cancel_callback=None):
    try:
        ...
        def progress_tracker():
            while is_waiting[0]:
                if cancel_callback and cancel_callback(): break
                elapsed = int(time.time() - start_time)
                if status_callback: status_callback(f"⏳ Voicebox is rendering full audio... Please wait ({elapsed}s)")
                time.sleep(1)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                except Exception as e: return False, f"Voicebox JSON Parse Error: {e}"
            else:
                is_waiting[0] = False
...
def generate_fish_speech_audio(text, server_url, presets, start_preset_name, output_path, status_callback=None, cancel_callback=None):
    ...
    base_url = (server_url or "").strip().rstrip('/')
    if "0.0.0.0" in base_url: base_url = base_url.replace("0.0.0.0", "127.0.0.1")
    preset_data = presets.get(start_preset_name, {})
    ref_audio_path = preset_data.get("audio", "").strip('\"\'')
    ref_text_path = preset_data.get("text", "").strip('\"\'')
    ...
    if ref_text_path and os.path.exists(ref_text_path):
        try:
            with open(ref_text_path, "r", encoding="utf-8") as f: ref_text_content = f.read().strip()
        except: pass
    ...
    def progress_tracker():
        while is_waiting[0]:
            if cancel_callback and cancel_callback(): break
            elapsed = int(time.time() - start_time)
            if status_callback: status_callback(f"⏳ Fish Speech is rendering... Please wait ({elapsed}s)")
            time.sleep(1)
    ...
    try:
        ...
        payload = {
             "text": text,
             "format": "wav",
             "references": [{"audio": audio_b64, "text": ref_text_content}],
             "normalize": True
        }
        headers = {"Content-Type": "application/json"}
        ...
        if response.status_code == 200:
             if response.headers.get('Content-Type', '').startswith('audio/'):
                 ...
                 try:
                     ...
                     if b64:
                         with open(output_path, "wb") as out_f: out_f.write(base64.b64decode(b64))
                         return True, output_path, start_preset_name
                 except: pass
...
def main(page: ft.Page):
    ...
    chk_flet_audio = ft.Checkbox(label="Enable Built-in Audio Player (Uncheck if app crashes)", value=False)
    ...
    def on_audio_state_changed(e):
        if hasattr(e, 'data') and e.data == "completed":
            ...
            if is_video_recording[0]: is_video_recording[0] = False
            try: 
    ...
    chk_flet_audio.value = app_settings.get("flet_audio_enabled", False)
    ...
    def set_loading(is_loading, msg="Ready", is_rendering_audio=False):
        ...
        if not is_audio_playing[0]:
            ...
            chk_flet_audio.disabled = is_loading
    ...
    def save_app_settings(e=None):
        try:
            ...
            if hasattr(chk_flet_audio, 'value'): data["flet_audio_enabled"] = chk_flet_audio.value
    ...
    for dd in [dd_format, dd_style, dd_theme, dd_length, dd_lang, dd_duration, dd_tts_engine, chk_autoplay, chk_force_cache, chk_flet_audio, tf_tiktok_session, dd_tiktok_voice, tf_voicebox_url, tf_voicebox_preset, dd_voicebox_engine, dd_elevenlabs_preset, dd_fish_preset, tf_fish_url]:
    ...
    def on_play_tts(e=None, record_video=False):
        ...
        if is_audio_playing[0]:
            try:
                ...
                is_pc_env = page.platform not in [ft.PagePlatform.ANDROID, ft.PagePlatform.IOS]
                
                if PYGAME_AVAILABLE and is_pc_env and pygame.mixer.get_init() and pygame.mixer.music.get_busy(): 
                ...
                if success:
                    ...
                    try:
                        ...
                        def ui_updates():
                            ...
                            if not is_silent_playback:
                                is_pc = page.platform not in [ft.PagePlatform.ANDROID, ft.PagePlatform.IOS]
                                
                                if PYGAME_AVAILABLE and is_pc:
                                    if not pygame.mixer.get_init(): pygame.mixer.init()
                                    pygame.mixer.music.load(output_path)
                                    pygame.mixer.music.play()
                                elif chk_flet_audio.value:
                                    try: 
                                        global audio_player
                                        if 'audio_player' not in globals() or not audio_player:
                                            if AudioControl:
                                                ...
                                                page.update()
                                        elif audio_player not in page.overlay:
                                            page.overlay.append(audio_player)
                                            page.update()
                                        ...
                                        if audio_player and getattr(audio_player, "page", None):
                                            ...
                                            if not is_pc and not safe_path.startswith("file://"):
                                    ...
                                    show_snack("🔊 Audio disabled! Enable 'Built-in Audio Player' in Setup tab after fixing requirements.txt.", ft.Colors.ORANGE)
    ...
    settings_tab_content = ft.Column([
        ...
        ft.Divider(),
        ft.Text("App Safety Settings", weight=ft.FontWeight.BOLD, size=18, color=ft.Colors.RED_400),
        ft.ResponsiveRow([
            ft.Column([chk_flet_audio], col={"sm": 12, "xs": 12}),
        ]),
        ft.Divider(),
